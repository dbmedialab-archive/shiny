...
const NoImageOnInitialLoadImagesStory = () => {
	const ids = [
		// 60375866,
		// 68797204,
		// 69761110,
	...
	return (
		ids.map((id, i) => (
			<LazyProgressiveImage
				ratio={0.5}
				fallbackSrc={`https://9.dbstatic.no/${id}.jpg?imageId=${id}&panow=0&panoy=0&panoh=0&panox=0&heightx=0&heightw=0&heighth=0&heighty=0&width=98&height=49&compression=30`}
				key={i}
			>
				<Source
...
export default withInfo('LazyProgressiveImage with no initial image load, then loads the correct image when it is ready')(NoImageOnInitialLoadImagesStory);
